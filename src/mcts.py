...
class MCTS:
    def __init__(self, chessboard, vloss, batch_size, policy):
        self.lib = CDLL('bazel-bin/mcts/libcapi.so')
        ...
        def callback(n, chessboards, probs, vs):
            i = np.stack([
                self._byte_ptr_to_chessboard(chessboards[i])
                for i in range(n)
            ])
            x, y = policy(i)
            for i in range(n):
        ...
        self.lib.MCTS_new.argtypes = \
            [char_arr_t, c_double, c_int, callback_t]
        ...
        self.lib.MCTS_GetPi.argtypes = \
            [c_void_p, c_double, POINTER(c_double)]
    ...
    def get_pi(self, temperature):
        ...
        self.lib.MCTS_GetPi(
            self.handle, c_double(temperature),
            cast(pi, POINTER(c_double))
        )
